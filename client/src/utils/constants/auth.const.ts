...
export const nextauthOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    jwt: async (arg) => {
      ...
      const { token, user, account, session, trigger } = arg

      if (trigger === 'update') {
        // in this case 'session' value is equal to the argument passed to the 'update' function
    ...
    session: async (arg) => {
      ...
      session.token = { provider, sessionId }
