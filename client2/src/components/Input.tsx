...
export default function FormInput({
  ...
  const handleChange = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (setIconVisibility) setIconVisibility(!changeIcon);
  };

  const styles = {
    wrapperInput: clsx(
      "flex justify-between rounded-full border border-solid border-gray-300 px-4 py-3",
    ),
    input: clsx("w-full min-w-0 border-none outline-none outline-0"),
    textArea: clsx(
      "w-full min-w-0 rounded-3xl border border-solid border-gray-300 px-4 py-3",
    ),
    error: clsx(
      "gap-estilo4 smalltext ml-2 flex font-medium text-red-700 dark:text-red-800",
    ),
  };
  ...
  return (
    <label
      htmlFor={name}
      className={`smalltext flex w-full min-w-0 flex-col gap-1 font-normal ${labelClass}`}
    >
      {label}
      {type !== "textarea" && type !== "select" && type !== "file" ? (
        <div className={`${wrapperIntupClassName} ${styles.wrapperInput}`}>
          ...
          <button onClick={handleChange}>
            {icon_1 && icon_2 ? (changeIcon ? icon_1 : icon_2) : icon_1}
          </button>
        </div>
      ) : type === "file" ? (
        <div className={`${wrapperIntupClassName} ${styles.wrapperInput}`}>
